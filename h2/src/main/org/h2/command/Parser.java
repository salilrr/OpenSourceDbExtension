	...
	private TableFilter readJoin(TableFilter top, Select command,
		...
		while (true) {
			if (readIf("RIGHT")) {
				...
				// throw getSyntaxError();
				/*
				 * Author Dbsi FOr full outer join use left outer join and
				 * indicate in the join by setting containsOuterJoin flag to
				 * true
				 */
				...
				top.addJoin(join, true, false, on);
				join.setContainsFullOuterJoin(true);
				last = join;
	...
	private Expression readAggregate(int aggregateType, String aggregateName) {
		...
		if (aggregateType == Aggregate.COUNT) {
			...
			if (equalsToken("GROUP_CONCAT", aggregateName)) {
				agg = new Aggregate(Aggregate.GROUP_CONCAT, readExpression(),
